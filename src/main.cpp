...
#if defined(NDEBUG)
# error "Gemoneycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf02bdcd158b2a21a65bed86963d3cfa8c267ad3faaceecff193776495cf046a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Gemoneycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Gemoneycoin Signed Message:\n";
...
{
    // Gemoneycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Gemoneycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Gemoneycoin: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Gemoneycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Gemoneycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Gemoneycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("7075b6106d47835b0a6bdfe46ff1783ace3016edf1d35b2436625acc0d47ebe1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 05/22/22 Ukrain peace deal: Kyiv rules out ceding land to Russia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f6a8adc64fb7f8108ed992ed45156802740d2d35b986759c941c880798670b74a077d85f95a5d1e698e935933b37a4a50bcf9571bc3db7d18efdbdf001249b20") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1653664248;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085060592;

        if (fTestNet)
        {
            block.nTime    = 1653664193;
            block.nNonce   = 388005845;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x12b8beecd53f5097872ef7556e9209343f3c2251a84eb3b1c3934da944153e73"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfC, 0xc9, 0xbE, 0xdA }; // Gemoneycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GemoneycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GemoneycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GemoneycoinMiner(CWallet *pwallet)
{
    printf("GemoneycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gemoneycoin-miner");
    ...
    try { loop {
        ...
        printf("Running GemoneycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GemoneycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GemoneycoinMiner, pwallet));
}
